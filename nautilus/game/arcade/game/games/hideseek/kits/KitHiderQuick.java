...
public class KitHiderQuick
  extends Kit
{
  public KitHiderQuick(ArcadeManager manager)
  {
    super(manager, "Instant Hider", KitAvailability.Green, new String[] {"Changes into solid blocks almost instantly!" }, new Perk[0], EntityType.SLIME, new ItemStack(Material.FEATHER));
  }
  ...
  public void ToBlock(final DisguiseBlock block, final DisguiseSlime slime)
  {
    ...
    UtilServer.getServer().getScheduler().scheduleSyncDelayedTask(this.Manager.GetPlugin(), new Runnable()
    {
      public void run()
      {
        KitHiderQuick.this.ToSlime(block, slime);
      }
    }, 60L);
  }
  ...
  public void ToSlime(final DisguiseBlock block, final DisguiseSlime slime)
  {
    ...
    UtilServer.getServer().getScheduler().scheduleSyncDelayedTask(this.Manager.GetPlugin(), new Runnable()
    {
      public void run()
      {
        KitHiderQuick.this.ToBlock(block, slime);
      }
    }, 60L);
  }
}
