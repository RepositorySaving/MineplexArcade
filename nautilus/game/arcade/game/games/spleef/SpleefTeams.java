...
public class SpleefTeams
  extends TeamGame
{
  public SpleefTeams(ArcadeManager manager)
  {
    super(manager, GameType.SpleefTeams, new Kit[] {new KitLeaper(manager), new KitBrawler(manager), new KitArcher(manager) }, new String[] {"Punch blocks to break them!", "1 Hunger per block smashed!", "Last team alive wins!" });
    ...
    this.Manager.GetPlugin().getServer().getScheduler().scheduleSyncDelayedTask(this.Manager.GetPlugin(), new Runnable()
    {
      public void run()
      {
        try
        {
          ...
          SpleefTeams.this.BlockFade(block, this.val$player);
          
          for (Block other : UtilBlock.getInRadius(block.getLocation().add(0.5D, 0.5D, 0.5D), radius).keySet())
          {
            SpleefTeams.this.BlockFade(other, this.val$player);
          }
