...
public class PerkSquidRifle extends Perk implements mineplex.core.projectile.IThrown
{
  ...
  {
    super("Ink Blaster", new String[] {C.cYellow + "Right-Click" + C.cGray + " with Axe to use " + C.cGreen + "Ink Blaster" });
  }
  ...
  public void Shoot(PlayerInteractEvent event)
  {
    ...
    if (!Recharge.Instance.use(player, GetName(), 1500L, true, true)) {
      return;
    }
    ...
    FireworkEffect effect = FireworkEffect.builder().flicker(false).withColor(Color.YELLOW).with(org.bukkit.FireworkEffect.Type.BALL).trail(false).build();
    
    try
    {
      Vector vel = player.getLocation().getDirection().multiply(1.8D);
